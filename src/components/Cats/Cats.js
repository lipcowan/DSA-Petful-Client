...
export default class Cats extends Component {
    ...
    componentDidUpdate(preProps, preState) {
        if(this.context.catsQue !== null){
            if(preState.length !== this.context.catsQue.length){
                this.setState({
                    index: 0,
    ...
    moveRight = () => {
        if(this.state.index < this.context.catsQue.length - 1) {
            this.setState({
                index: this.state.index + 1
            })
        }
    ...
    render() {
        if(this.context.catsQue === null) {
            return (
                <h2> Loading available cats ...</h2>
            )
        } else if(!this.context.catsQue.length) {
            return (
                <div className="allGone">
                    <h2>We're sorry, all cat's have been adopted</h2>
                    <h3>Please check back again tomorrow - thanks!</h3>
                </div>
            )
        } else {
            const props = this.context.catsQue[this.state.index];
            return (
